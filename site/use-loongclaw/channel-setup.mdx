...
smoke tests and worked examples instead, start with [Channel Guides](/use-loongclaw/channel-guides/index) for the exact per-surface contract, then keep [Channel Recipes](/use-loongclaw/channel-recipes) for representative rollout patterns.
If you want the shared public config shape before the per-surface setup rules,
start with [Configuration Patterns](/use-loongclaw/configuration-patterns).
...
| the shared account and selector config shape first | [Configuration Patterns](/use-loongclaw/configuration-patterns) |
