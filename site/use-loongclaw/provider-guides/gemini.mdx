---
title: Gemini
description: Configure the Gemini provider profile in LoongClaw.
---

# Gemini

Use this when your team already runs on Gemini and you want that vendor family to stay explicit in LoongClaw config instead of disappearing behind a generic compatibility route.

## At A Glance

| Field | Value |
| --- | --- |
| Built-in kind | `gemini` |
| Provider group | Direct Hosted Providers |
| Protocol family | `openai_chat_completions` |
| Feature family | `openai_compatible` |
| Auth scheme | `bearer` |
| Credential envs | GEMINI_API_KEY, GOOGLE_API_KEY |
| Aliases | `gemini_compatible`, `google`, `google_gemini`, `google-gemini` |
| Default base URL | `https://generativelanguage.googleapis.com/v1beta/openai` |
| Request endpoint | `https://generativelanguage.googleapis.com/v1beta/openai/chat/completions` |
| Models endpoint | `https://generativelanguage.googleapis.com/v1beta/openai/models` |

## Minimal Config

```toml
active_provider = "gemini"

[providers.gemini]
kind = "gemini"
api_key = { env = "GEMINI_API_KEY" }
model = "auto"
```

## Verify It

```bash
loong doctor
loong list-models
loong ask --message "Say hello and name the active provider."
```

If `list-models` is unreliable for this account or region, pin an explicit `provider.model` or add `preferred_models` instead of leaving recovery implicit.

## Auth And Routing Contract

| Contract | Value |
| --- | --- |
| Auth optional | no |
| Model probe auth optional | no |
| Default API key env | `GEMINI_API_KEY` |
| OAuth env | none |
| Primary request route | `https://generativelanguage.googleapis.com/v1beta/openai/chat/completions` |
| Primary model-catalog route | `https://generativelanguage.googleapis.com/v1beta/openai/models` |

## Related Docs

- Continue to [Providers And Models](/use-loongclaw/providers-and-models) when you still need the broader chooser page.
- Continue to [Provider Guides](/use-loongclaw/provider-guides/index) for the full built-in provider matrix.
- Continue to [Provider Recipes](/use-loongclaw/provider-recipes) for representative rollout recipes.
- Continue to [Configuration Patterns](/use-loongclaw/configuration-patterns) for the shared provider-profile shape.
